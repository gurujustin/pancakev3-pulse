...
import { bscProvider, pulseTestnetProvider } from './provider'
...
const cakeBusdPairMap = {
  ...
  [ChainId.PULSE_TESTNET]: {
    address: Pair.getAddress(CAKE[ChainId.PULSE_TESTNET], BUSD[ChainId.PULSE_TESTNET]),
    tokenA: CAKE[ChainId.PULSE_TESTNET],
    tokenB: BUSD[ChainId.PULSE_TESTNET],
  },
}

const getCakePrice = async (isTestnet: boolean) => {
  const pairConfig = cakeBusdPairMap[isTestnet ? ChainId.PULSE_TESTNET : ChainId.BSC]
  const pairContract = new Contract(pairConfig.address, pairAbi, isTestnet ? pulseTestnetProvider : bscProvider)
