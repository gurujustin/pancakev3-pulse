...
describe('transaction reducer', () => {
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.PULSE_TESTNET]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllChainTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.BSC), String(ChainId.PULSE_TESTNET)])
      expect(Object.keys(store.getState()[ChainId.BSC] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.PULSE_TESTNET] ?? {})).toEqual(['0x1'])
